...
func TestEmailProcessor(t *testing.T) {
    ...
    err := emailProcessor(context.Background(), event)
...
func TestEmailProcessorWithMock(t *testing.T) {
    mockSender := &MockEmailSender{}
    handler := createEmailProcessor(mockSender)
...
func TestSyncPublish(t *testing.T) {
    ...
    assert.True(t, executed, "Processor should execute immediately in sync mode")
}
```
...
func TestRetryMiddleware(t *testing.T) {
    ...
    wrappedProcessor := gossip.WithRetry(3, 10*time.Millisecond)(handler)
    
    err := wrappedProcessor(context.Background(), gossip.NewEvent(UserCreated, nil))
...
func TestEventFilter(t *testing.T) {
    ...
    filter := func(e *gossip.Event) bool {
        return e.Metadata["source"] == "api"
    }

    handler := gossip.NewFilteredProcessor(filter, func(ctx context.Context, e *gossip.Event) error {
        executed = true
        return nil
    })

    // Non-api source - should not execute
    event := gossip.NewEvent(UserCreated, nil).WithMetadata("source", "web")
    handler(context.Background(), event)
    assert.False(t, executed)

    // API source - should execute
    event = gossip.NewEvent(UserCreated, nil).WithMetadata("source", "api")
    handler(context.Background(), event)
...
func TestBatchProcessor(t *testing.T) {
    ...
    processor := gossip.NewBatchProcessor(OrderCreated, config, batchProcessor)
