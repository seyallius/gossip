...
// metricsProcessor tracks authentication metrics.
func metricsProcessor(ctx context.Context, event *gossip.Event) error {
	log.Printf("[Metrics] Incrementing counter: %s", event.Type)
	return nil
}

// securityProcessor handles security-related logic.
func securityProcessor(ctx context.Context, event *gossip.Event) error {
	switch event.Type {
	case UserLoggedIn:
		data := event.Data.(*LoginData)
		if !data.Success {
...
func NewAuthService(bus *bus.EventBus) *AuthService {
...
func main() {
	...
	eventBus.Subscribe(UserRegistered, auditLogProcessor)
	eventBus.Subscribe(UserRegistered, metricsProcessor)

	eventBus.Subscribe(UserLoggedIn, emailNotificationProcessor)
	eventBus.Subscribe(UserLoggedIn, auditLogProcessor)
	eventBus.Subscribe(UserLoggedIn, metricsProcessor)
	eventBus.Subscribe(UserLoggedIn, securityProcessor)

	eventBus.Subscribe(PasswordChanged, emailNotificationProcessor)
	eventBus.Subscribe(PasswordChanged, auditLogProcessor)
