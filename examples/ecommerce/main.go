...
func main() {
	...
	batchProcessor := gossip.NewBatchProcessor(OrderCreated, batchConfig, batchEmailProcessor)
	...
	bus.Subscribe(OrderPaid, paymentProcessor)
	bus.Subscribe(OrderShipped, shippingProcessor)

	// Batch handler for emails
	bus.Subscribe(OrderCreated, batchProcessor.AsEventProcessor())
	...
	bus.Subscribe(OrderCreated, gossip.NewFilteredProcessor(highValueFilter, analyticsProcessor))
